...
function verify(txBase64) {
  ...
  if (!isValidTx) {
    throw new Error('stellar-auth.errors.invalid-transaction')
  }

  if (!hasValidClientSignature(tx)) {
    throw new Error('stellar-auth.errors.invalid-signature');
  }

  if (!hasValidTimeBounds(tx)) {
    throw new Error('stellar-auth.errors.expired-transaction')
  }
